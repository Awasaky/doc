...
Each bean is then responsible for sending status messages to the manager: "contact" if it spots an enemy or "ouch!" if it is hit and takes damage. For this to work, the bean controller scripts use absolute addressing to send messages to the component "controller" in "manager".
